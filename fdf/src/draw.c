...
/*   Updated: 2022/10/10 00:48:38 by jpedrones        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	bresenham(t_image *img, t_point init, t_point dest)
{
	...
	{
		if (bres.x > X_WIN || bres.y > Y_WIN || bres.x < 0 || bres.y < 0)
			break ;
...
t_point	define_point(t_map *map, t_point p)
{
	...
	p.y += (Y_WIN + (5 * map->cam.zoom)) / 2 + map->cam.y_offset;
	return (p);
}
